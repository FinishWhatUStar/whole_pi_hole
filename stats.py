...
smallfont = ImageFont.truetype('/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf', 17)
...
while True:
    ...
    if buttonA.value and not buttonB.value:  # just button B pressed
        ...
        draw.rectangle((0, 0, width, height), outline=0, fill=(15, 25, 40))
        backlight.value = True
        draw.text((x, y), IP, font=smallfont, fill="#CCEEF9")
        y += smallfont.getsize(IP)[1]
        draw.text((x, y), "Ads Blocked: {}".format(str(ADSBLOCKED)), font=smallfont, fill="#B6E6F6")
        y += smallfont.getsize(str(ADSBLOCKED))[1]
        draw.text((x, y), "DNS Queries: {}".format(str(DNSQUERIES)), font=smallfont, fill="#9BDDF2")
        y += smallfont.getsize(str(DNSQUERIES))[1]
        draw.text((x, y), CPU, font=smallfont, fill="#6FCEED")
        y += smallfont.getsize(CPU)[1]
        draw.text((x, y), "Memory:", font=smallfont, fill="#3FBEE7")
        y += smallfont.getsize(MemUsage)[1]
        draw.text((x, y), MemUsage, font=smallfont, fill="#0CADE1")
        y += smallfont.getsize(MemUsage)[1]
        draw.text((x, y), Disk, font=smallfont, fill="#009DD8")
        y += smallfont.getsize(Disk)[1]
        draw.text((x, y), Temp, font=smallfont, fill="#008ECE")
